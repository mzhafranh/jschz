...
/* GET home page. */
module.exports = function (MongoClient, urlDb) {
    MongoClient.connect(urlDb, function (err, db) {
        ...
        function add(id, string, integer, float, date, boolean, callback) {
            ...
            dbo.collection("data").insertOne(noSql, (err, res) => {
                callback(err);
            });
        }

        function select(id, callback) {
            dbo.collection("data").find({ "id": parseInt(id) }).toArray(function (err, res) {
                callback(err, res);
            });
        }
        ...
        function update(newId, oldId, string, integer, float, date, boolean, callback) {
            ...
            noSql += '}'
            noSql = JSON.parse(noSql);
            dbo.collection("data").updateOne({ "id": parseInt(oldId) }, { $set: noSql }, (err, res) => {
                callback(err);
            });
        }

        function remove(id, callback) {
            dbo.collection("data").deleteOne({ id: parseInt(id) }, (err, res) => {
                callback(err);
            })
        }
        ...
        router.get('/', async function (req, res,) {
            ...
            dbo.collection("data").find(noSql).toArray(function (err, result) {
                if (err) {
                    console.error(err);
                }
                var total = result.length;
                const pages = Math.ceil(total / limit)
                dbo.collection("data").find(noSql).skip(offset).limit(limit).sort(sortMongo).toArray(function (err, result) {
                    if (err) {
                        console.error(err)
                    }
                    res.render('list', { rows: result, pages, page, filter, query: req.query, sortBy, order })
                })
            })
        })
